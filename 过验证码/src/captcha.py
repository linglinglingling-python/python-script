...
class TextSelectCaptcha(object):
    def __init__(self, per_path='pre_model_v2.bin', yolo_path='best.onnx', sign=True):
        """
    ...
    def run(self, image_path, input_chars=None):
        ...
        if len(targets)>len(chars):
            targets=targets[:len(chars)]
        result = []
            ...
            if len(targets) == 0:
                ...
                for n, target in enumerate(targets):
                    ...
                    for i in range(0,360,20):
                        img_target = img_target.rotate(i)
                        similarity = self.pre.reason(img_char, img_target)
                        simlar.append(similarity)
                    slys_index1 = simlar.index(max(simlar))
                    slys.append(simlar[slys_index1])
